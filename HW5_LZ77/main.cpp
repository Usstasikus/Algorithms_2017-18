...
using namespace std;
int findMaxSubstr(string &s, int dictBuf, int currSymb, int prevBufMax, vector<Node> &res);
...
void encodeLZ77(string& s, vector<Node>& res, int dictBufMax, int prevBufMax)
{
    ...
    {
        int len = findMaxSubstr(s, dictBuf, i, prevBufMax, res);
        i += len;
...
int findMaxSubstr(string &s, int dictBuf, int currSymb, int prevBufMax, vector<Node> &res)
{
    ...
    while(indexSubstr < buffEnd) //максимальная подстрока должна начинаться в словаре
    {
        ...
        while (s[indexSubstr] == s[currSymb] && len <= prevBufMax)
        {
            ++indexSubstr;
            ++currSymb;
            ++len;
